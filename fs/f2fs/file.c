...
int f2fs_sync_file(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	// 		if(ss){
	// 			if(dentry->d_inode != NULL && dentry->d_inode->i_sb != NULL && 
	// 						dentry->d_inode->i_sb->s_type != NULL && dentry->d_inode->i_sb->s_type->name != NULL){
	// 				if(strcmp(dentry->d_inode->i_sb->s_type->name, "f2fs") == 0 && is_inode_flag_set(F2FS_I(inode), FI_RAMFS)){
	// 					do_gettimeofday(&tv);
	// 					printk(KERN_ALERT "->%ld,fsync,f2fs,%s\n", (tv.tv_sec * 1000 + tv.tv_usec / 1000), ss);
	// 				}
	// 			}
	// 		}
	// 		kfree(fname);
...
static ssize_t f2fs_file_read_iter(struct kiocb *iocb, struct iov_iter *from)
{
       ...
       if(is_inode_flag_set(fi, FI_RAMFS)) {
    	   now = get_cur_time();
