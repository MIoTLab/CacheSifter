...
// #define CONFIG_Lenny_TRACE_META
#ifdef CONFIG_Lenny_TRACE_META
 	if(bio){
		if(is_read_io(fio->rw)) {                                         
			...
			if(page->mapping) {
				...
				if (inode && inode->i_dentry.first) {
					next = inode->i_dentry.first;
					dentry = hlist_entry(next, struct dentry, d_u.d_alias);
					fname = (char *) kmalloc(sizeof(char) * 256, GFP_KERNEL);
					ss = dentry_path_raw(dentry, fname, 256);
					if (ss) {
						if(strncmp(ss, "/data/", 6) == 0 || strncmp(ss, "/app/", 5) == 0 || strncmp(ss, "/media/", 7) == 0) {
							if(strstr(ss, "/cache/")) {
								printk(KERN_ALERT "[filemgr] %s,wbio,%u\n", ss,bio->bi_iter.bi_size);
							}
						}
						kfree(fname);
					}
...
// #define CONFIG_Lenny_TRACE_META1
#ifdef CONFIG_Lenny_TRACE_META1
	if(pending_bio){
		if(is_read_io(fio->rw)) {                                         
			...
			if(bio_page->mapping) {
				...
				if (inode && inode->i_dentry.first) {
					...
					if (ss) {
						if(strncmp(ss, "/data/", 6) == 0 || strncmp(ss, "/app/", 5) == 0 || strncmp(ss, "/media/", 7) == 0) {
							if(strstr(ss, "/cache/")) {
								printk(KERN_ALERT "[filemgr] %s,wbio,%u\n", ss, pending_bio->bi_iter.bi_size);
							}
						}
						kfree(fname);
					}
