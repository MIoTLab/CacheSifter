...
void write_data_page(struct dnode_of_data *dn, struct f2fs_io_info *fio)
{
	...
	// if(is_inode_flag_set(F2FS_I(dn->inode), FI_RAMFS)) {
	// 	static unsigned int nr_bar = 0, nr_mem = 0, nr_cln = 0;
	// 	if (is_inode_ftype(dn->inode, INODE_UNEVICTABLE)){
	// 		nr_mem++;
	// 	}
	// 	else if (is_inode_ftype(dn->inode, INODE_CLEAN)) {
	// 		nr_bar++;
	// 	}
	// 	else {
	// 		nr_cln++;
	// 	}
	// 	printk(KERN_ALERT "[filemgr] write page nr_bar = %u, nr_mem = %u, nr_cln = %u,\n", nr_bar, nr_mem, nr_cln);
	// }
