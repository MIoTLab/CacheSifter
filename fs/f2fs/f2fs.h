...
enum {
    INODE_INIT,
    INODE_UNEVICTABLE, // 加一个flag，删flash空间，增加ramfs空间
    INODE_WRITEBACK, // 什么都不改，
    INODE_CLEAN, // 设置为uptodate，删除inode，删flash空间
	INODE_STAGE1,
};
...
struct filemgr_list {
	...
	unsigned long active_used_bytes; // 在active list保存了多少字节的数据
...
{
       return CURRENT_TIME_SEC.tv_sec;
}
...
#ifdef CONFIG_F2FS_RAMFS
	FI_RAMFS,           /* store file in ramfs without wirteback */
	FI_RAMFS_DELETED,           /* store file in ramfs without wirteback */
#endif
};
