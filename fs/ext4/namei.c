	...
	if(dentry != NULL){
		...
		if(ss){
			if(dentry->d_inode != NULL && dentry->d_inode->i_sb != NULL && dentry->d_inode->i_sb->s_type != NULL && dentry->d_inode->i_sb->s_type->name != NULL){
				if(strcmp(dentry->d_inode->i_sb->s_type->name, "ext4") == 0 && current != NULL){
					do_gettimeofday(&tv);
					...
					printk(KERN_ALERT "->%ld,unlink,ext4,%s,%ld,%ld,%ld,%lld,%lu,%d,%s\n", (tv.tv_sec * 1000 + tv.tv_usec / 1000), ss,
							(ttnow - atime), (ttnow - mtime), (ttnow - ctime), dentry->d_inode->i_size, dentry->d_inode->i_ino, current->pid, current->comm,);
				}
			}
		}
		kfree(fname);
