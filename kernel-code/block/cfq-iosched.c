...
/*{ //lenny
		if(rq->bio){
			...
			char * fname;
			char * ss;
			if( bio_data_dir(rq->bio)== WRITE){ //bio_data_dir(rq->bio)
                                
				if(rq->bio->bi_io_vec && rq->bio->bi_io_vec->bv_page && rq->bio->bi_vcnt > 0){
						...
						if(page->mapping){
							...
							if(inode){ 
								if(inode->i_dentry.first){ 
									...
									if(ss){ 
										//if(strstr(ss, "/data/") || strstr(ss, "/app/") || strstr(ss, "/media/") ) {
											do_gettimeofday(&tv);
											printk(KERN_ALERT "**>[%d]%ld,%s,cfq-bio-w,%lu,%u,%lu,%d\n", i, (tv.tv_sec * 1000 + tv.tv_usec / 1000), ss, inode->i_ino, rq->bio->bi_iter.bi_size, rq->bio->bi_iter.bi_sector, rq->bio->bi_vcnt);
										//}
									}
									kfree(fname);
								}
							...
							if(inode){ 
								if(inode->i_dentry.first){ 
									...
									if(ss){ 
										//if(strstr(ss, "/data/") || strstr(ss, "/app/") || strstr(ss, "/media/") ) {
											do_gettimeofday(&tv);
											printk(KERN_ALERT "**>[%d]%ld,%s,cfq-bio-r,%lu,%u,%lu,%d\n", i, (tv.tv_sec * 1000 + tv.tv_usec / 1000), ss, inode->i_ino, rq->bio->bi_iter.bi_size, rq->bio->bi_iter.bi_sector, rq->bio->bi_vcnt);
										//}
									}
									kfree(fname);
								}
