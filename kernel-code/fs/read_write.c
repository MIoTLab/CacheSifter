...
//#define CONFIG_Lenny_TRACE_READ
        ...
        if (current->cred->uid.val == 10086) //filter app by using uid of youtube
	{
		...
		if(filp != NULL && filp->f_path.dentry != NULL) {                      
				...
				if(ss && current != NULL) {
					do_gettimeofday(&tv);
					printk(KERN_ALERT "->%ld,vfs_r,%s,%lu,%lld,%lld,%lu,%d,%s\n",
							(tv.tv_sec * 1000 + tv.tv_usec / 1000),
							ss, len, filp->f_inode->i_size, *ppos, filp->f_inode->i_ino,current->pid, current->comm);
				}
			    } // directory filter
			    kfree(fname);                     
		} //dentry filter
...
//#define CONFIG_Lenny_TRACE_WRITE
        ...
        if (current->cred->uid.val == 10086) //filter app by using uid of youtube
	{
		...
		if(filp != NULL && filp->f_path.dentry != NULL) {                       
				...
				if(ss && current != NULL) {
					do_gettimeofday(&tv);
					printk(KERN_ALERT "->%ld,vfs_w,%s,%lu,%lld,%lld,%lu,%d,%s\n",
							(tv.tv_sec * 1000 + tv.tv_usec / 1000),
							ss, len, filp->f_inode->i_size, *ppos, filp->f_inode->i_ino,current->pid, current->comm);
				}
			    } // directory filter
