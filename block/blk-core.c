...
void submit_bio(int rw, struct bio *bio)
{
...
#if ((defined CONFIG_VFS_TRACE_BIO_READ) || (defined CONFIG_VFS_TRACE_BIO_WRITE))
	{
			if (bio) {
				...
				char * fname;
				char * ss;
				if (bio->bi_rw == READ) { // 必须使用读，因为写有多种情况
									...
									if (inode->i_dentry.first) {
										...
										if (ss) {
											if (TRACE_CONDITION) {
												...
												if(mapping_unevictable(page->mapping))
													is_uevi = 1;
												...
												do_gettimeofday(&tv);
												/*printk(
													KERN_ALERT "[filemgr]%ld,wbio,%s,%u,%u,%lu,%lld,%lu, is_dir = %d, is_reg = %d, is_unevi = %d\n",
													(tv.tv_sec * 1000 + tv.tv_usec / 1000),
													ss, bio->bi_iter.bi_size,
													bio->bi_vcnt,
													bio->bi_iter.bi_sector,
													inode->i_size, inode->i_ino, 
													is_dir, is_reg, is_uevi
													);*/
                                                                                                  printk(KERN_ALERT "->%s, bio-w, %u, %lld, %lu\n", ss, bio->bi_iter.bi_size, dentry->d_inode->i_size, dentry->d_inode->i_ino);
											}
										kfree(fname);
									}
									break;
